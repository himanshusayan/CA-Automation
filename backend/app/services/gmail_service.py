...
class GmailService:
    ...
    async def get_registered_emails(self, db: AsyncSession) -> List[str]:
        ...
        try:
            ...
            client_emails = result.scalars().all()
            return [email.email for email in client_emails if email.company and email.company.is_active]
        except Exception as e:
            logger.error(f"Failed to get registered emails: {e}")
            return []
    ...
    async def _process_email(self, db: AsyncSession, message_id: str, sender_email: str):
        """Process individual email with automatic AI classification"""
        try:
            # Check if email already exists
            existing = await db.execute(
                select(Email).where(Email.message_id == message_id)
            ...
            # AUTOMATIC AI CLASSIFICATION using Perplexity API
            logger.info(f"Starting AI classification for email {email_record.id}")
            classification_success = await perplexity_service.classify_email(db, email_record)
            
            if classification_success:
                logger.info(f"Email {email_record.id} classified as {email_record.primary_category}")
            else:
                logger.warning(f"AI classification failed for email {email_record.id}")
            
            # Mark as processed
            email_record.is_processed = True
            await db.commit()
            
            logger.info(f"Email {message_id} processing completed successfully")
    ...
    async def _get_company_from_email(self, db: AsyncSession, email_address: str) -> Optional[Company]:
        ...
        try:
            ...
            if client_email and client_email.company and client_email.company.is_active:
                return client_email.company
            else:
                logger.warning(f"No active company found for email: {email_address}")
                return None
    ...
    async def _process_attachments(self, db: AsyncSession, message: dict, email_record: Email):
        """Process and save email attachments with CA folder structure"""
        try:
            payload = message['payload']
    ...
    async def _save_attachment(self, db: AsyncSession, message: dict, part: dict, email_record: Email):
        """Save individual attachment with CA folder structure"""
        try:
            attachment_id = part['body'].get('attachmentId')
            if not attachment_id:
                return
            ...
            # Save using CA folder structure if company and classification available
            if email_record.company and email_record.primary_category:
                # Use AI-detected data period or current date as fallback
                data_month = email_record.data_month or datetime.now().month
                data_year = email_record.data_year or datetime.now().year
                ...
                # Save to temporary location until classification is done
                temp_dir = os.path.join(settings.STORAGE_BASE_PATH, "temp")
                os.makedirs(temp_dir, exist_ok=True)
                file_path = os.path.join(temp_dir, f"email_{email_record.id}_{filename}")
            ...
            db.add(attachment_record)
            logger.info(f"Attachment saved: {filename} for email {email_record.id}")
