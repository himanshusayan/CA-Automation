...
from app.models import Company, ClientEmail, Email, Reminder
...
async def list_companies(
    ...
    """List companies with proper active/inactive filtering"""
    try:
        query = select(Company).options(selectinload(Company.client_emails))
...
async def update_company(
    ...
    try:
        ...
        if company_update.client_emails is not None:
            ...
            await db.flush()  # Ensure deletions are committed
        ...
        # Return updated company with relationships loaded
        result = await db.execute(
            select(Company)
            .options(selectinload(Company.client_emails))
            .where(Company.id == company_id)
...
async def get_company_stats(company_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error(f"Failed to get stats for company {company_id}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to retrieve company stats: {str(e)}")